...
import {  Button, Card, Icon, Table, Feed,Comment, Form } from "semantic-ui-react";
...
function TableExampleCollapsing(){
  ...
  const sendChat = () =>{
    ...
    fetch("https://saosa.herokuapp.com/api/Bizmod/send", requestOptions)
      .then(response => response.json())
      .then(result =>{
        let temp = chats
        temp.push(result)
  ...
  const apply = (id) =>{
    ...
    fetch("https://saosa.herokuapp.com/api/Bizmod/get-chats?id="+id, requestOptions)
      .then(response => response.json())
  ...
  <div style={{ marginTop: 30,display:'flex' }}>
    ...
    <Card style={{width:500, marginLeft:10}}>
      ...
      <Comment.Group>
            ...
            <Card.Content extra>
            <Form reply>
            <Form.TextArea onChange={ val => setMessage(val.target.value)} />
            <Button
                content='Add Reply'
                labelPosition='left'
                icon='edit'
